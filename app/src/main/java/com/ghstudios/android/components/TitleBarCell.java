...
public class TitleBarCell extends FrameLayout {
    ...
    /**
     * Sets the item hid text.
     * @param itemHidText
     */
    public void setItemHidText(String itemHidText) {
        titleHid.setText(itemHidText);
        maybeEnableHidText();
    }
    ...
    /**
     * Runs some logic to see if item hid text should be enabled.
     */
    private void maybeEnableHidText() {
        CharSequence hidText = titleHid.getText();
        if (hidText != null && hidText.length() > 0) {
            titleHid.setVisibility(VISIBLE);
        } else {
            titleHid.setVisibility(GONE);
        }
    }
}
