...
class WeaponCursor(c: Cursor) : CursorWrapper(c) {
    ...
    val weapon: Weapon
        get() {
            ...
            weapon.reach = getString(S.COLUMN_WEAPONS_REACH)
            ...
            if (getBloated()) {
                val multiplicator = when (weapon.wtype) {
                ...
                weapon.attack = (weapon.attack * multiplicator).toInt()
            }

            if (getElementTrueRaw()) {
                weapon.elementAttack = (weapon.elementAttack / 10)
                weapon.element2Attack = (weapon.element2Attack / 10)
            }
