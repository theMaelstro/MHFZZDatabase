...
public class CombiningCursor extends CursorWrapper {
	...
	public Combining getCombining() {
		...
		int combiner_fee = getInt(getColumnIndex(S.COLUMN_COMBINING_COMBINER));
		int alchemy = getInt(getColumnIndex(S.COLUMN_COMBINING_ALCHEMY));
		...
		combining.setCombinerFee(combiner_fee);
		combining.setAlchemy((alchemy));
	...
	private Item itemHelper(String prefix) {
		...
		if (getString(getColumnIndex(prefix + S.COLUMN_ITEMS_NAME)) != null) {
