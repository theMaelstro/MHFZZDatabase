...
class WeaponBowCoatings(val coatingCode: Int) {
    val power = hasCoating(1 shl 14)
    val explosive = hasCoating(1 shl 13)
    val impact = hasCoating(1 shl 12)
    val poison1 = hasCoating(1 shl 11)
    val poison2 = hasCoating(1 shl 10)
    val poison3 = hasCoating(1 shl 9)
    val poison4 = hasCoating(1 shl 8)
    val para1 = hasCoating(1 shl 7)
    val para2 = hasCoating(1 shl 6)
    val para3 = hasCoating(1 shl 5)
    val para4 = hasCoating(1 shl 4)
    val sleep1 = hasCoating(1 shl 3)
    val sleep2 = hasCoating(1 shl 2)
    val sleep3 = hasCoating(1 shl 1)
    val sleep4 = hasCoating(1 shl 0)
    ...
    val hasPoison = poison1 || poison2 || poison3 || poison4
    val hasPara = para1 || para2 || para3 || para4
    val hasSleep = sleep1 || sleep2 || sleep3 || sleep4
}
...
class Weapon : Item() {
    companion object {
        ...
        const val TONFA = "Tonfa"
        const val MAGNET_SPIKE = "Magnet Spike"
    ...
    fun initializeSharpness() {
        ...
        var sharpness1 = IntArray(9)
        var sharpness2 = IntArray(9)
        var sharpness3 = IntArray(9)
        ...
        while (strSharpness1.size <= 9) {
            strSharpness1.add("0")
        }
        while (strSharpness2.size <= 9) {
            strSharpness2.add("0")
        }
        while (strSharpness3.size <= 9) {
            strSharpness3.add("0")
        }
        ...
        for (i in 0..8) {
            try {
                ...
                sharpness1 = intArrayOf(0, 0, 0, 0, 0, 0, 0, 0, 0)
                break
            }
        }
        for (i in 0..8) {
            try {
                ...
                Log.v("ParseSharpness", "Error in sharpness $sharpness")
                sharpness2 = intArrayOf(0, 0, 0, 0, 0, 0, 0, 0, 0)
                break
            }
        }
        ...
        for (i in 0..8) {
            try {
                ...
                Log.v("ParseSharpness", "Error in sharpness $sharpness")
                sharpness3 = intArrayOf(0, 0, 0, 0, 0, 0, 0, 0,0)
                break
            }
        }
