...
class MonsterDetailViewModel(app : Application) : AndroidViewModel(app) {
    ...
    val rewardEXData = MutableLiveData<List<HuntingReward>>()
    val rewardLRData = MutableLiveData<List<HuntingReward>>()
    val rewardHRData = MutableLiveData<List<HuntingReward>>()
    val rewardGOUData = MutableLiveData<List<HuntingReward>>()
    val rewardGData = MutableLiveData<List<HuntingReward>>()
    val rewardZ1Data = MutableLiveData<List<HuntingReward>>()
    val rewardZ2Data = MutableLiveData<List<HuntingReward>>()
    val rewardZ3Data = MutableLiveData<List<HuntingReward>>()
    val rewardZ4Data = MutableLiveData<List<HuntingReward>>()
    ...
    fun setMonster(monsterId : Long): MonsterMetadata {
        ...
        loggedThread(name = "Monster Loading") {
            ...
            if (metadata?.hasExRank == true) rewardEXData.postValue(dataManager.queryHuntingRewardMonsterRank(monsterId, "EX").toList { it.huntingReward })
            ...
            if (metadata?.hasGouRank == true) rewardGOUData.postValue(dataManager.queryHuntingRewardMonsterRank(monsterId, "GOU").toList { it.huntingReward })
            ...
            if (metadata?.hasZ1Rank == true) rewardZ1Data.postValue(dataManager.queryHuntingRewardMonsterRank(monsterId, "Z1").toList { it.huntingReward })
            if (metadata?.hasZ2Rank == true) rewardZ2Data.postValue(dataManager.queryHuntingRewardMonsterRank(monsterId, "Z2").toList { it.huntingReward })
            if (metadata?.hasZ3Rank == true) rewardZ3Data.postValue(dataManager.queryHuntingRewardMonsterRank(monsterId, "Z3").toList { it.huntingReward })
            if (metadata?.hasZ4Rank == true) rewardZ4Data.postValue(dataManager.queryHuntingRewardMonsterRank(monsterId, "Z4").toList { it.huntingReward })
        }
